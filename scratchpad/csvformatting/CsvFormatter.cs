...
namespace csvformatting
{
	public class CsvFormatter
	{
		...
		public string Format_as_pure_Operation(string csvText) {
			var csvLines = csvText.Split('\n');
			var csvRecords = csvLines.Select (l => l.Split (';')).ToArray ();
			var colWidths = new int[csvRecords.First ().Length];
			for (var iCol = 0; iCol < colWidths.Length; iCol++)
				colWidths [iCol] = csvRecords.Max (r => r [iCol].Length);
			var formattedRecords = csvRecords.Select (r => string.Join ("|", r.Select((v, i) => v.PadRight(colWidths[i]))))
				.ToArray ();
